...
import { ShoppingBag, Menu, X, Sparkles, LayoutGrid } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-40 w-full backdrop-blur-md bg-[#FAF8F5]/90 border-b border-[#D9D3C7] transition-all">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
        ...
        {/* Right: MIX@MATCH Button & Cart Icon */}
        <div className="flex items-center gap-3">
          
          {/* Main Feature Action: MIX@MATCH Button */}
          <button
            onClick={() => handleLinkClick('#fit-guide')}
            className="hidden sm:inline-flex items-center gap-1.5 px-4 py-2 bg-[#2D5A27] hover:bg-[#23471E] text-white text-xs font-bold uppercase tracking-wider rounded-xl shadow-md hover:shadow-[#2D5A27]/25 transition-all active:scale-95 cursor-pointer"
          >
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#FAF8F5] border-b border-[#D9D3C7] px-6 py-5 shadow-xl animate-fade-in">
          <nav className="flex flex-col gap-4">
            ...
            <button
              onClick={() => handleLinkClick('#fit-guide')}
              className="mt-2 w-full py-2.5 bg-[#2D5A27] text-white text-xs font-bold uppercase tracking-wider rounded-xl flex items-center justify-center gap-2 shadow-md"
            >
              <Sparkles size={14} />
