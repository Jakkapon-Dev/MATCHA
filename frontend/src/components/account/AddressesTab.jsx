    ...
    <div className="bg-matcha-bg border border-matcha-border p-6 sm:p-8 space-y-6">
      ...
      {saved.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {saved.map((addr) => (
            <div key={addr.id} className="p-5 border border-matcha-border space-y-2 text-xs font-mono">
              <div className="flex items-center justify-between gap-2">
                <span className="font-bold text-[#0A0A0A]">
                  {addr.title || [addr.firstName, addr.lastName].filter(Boolean).join(' ')}
                </span>
                {addr.isDefault && (
                  <span className="px-2 py-0.5 bg-[#0A0A0A] text-matcha-bg text-[10px] font-bold uppercase shrink-0">
              ...
              {addr.phone && (
                <div className="text-matcha-muted">{t('checkout.tel')}: {addr.phone}</div>
              )}
            </div>
          ))}
        </div>
