...
export default function useLookbooks() {
  ...
  useEffect(() => {
    ...
    mediaRequest('/lookbooks').then(result => { if (active) setLooks(result.data); })
      .catch(() => { if (active) { setLooks(preview); setError('โหลดข้อมูลสินค้ายังไม่สำเร็จ ขณะนี้แสดงภาพลุคเดิมและพักการสั่งซื้อไว้'); } })
