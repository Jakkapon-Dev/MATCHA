...
export default function DemoCheckout() {
  ...
  return <section className="max-w-3xl mx-auto p-5 sm:p-10 my-8 bg-white border border-[#D9D3C7] rounded-2xl text-[#2D231E]">
    ...
    {order ? <div role="status" className="space-y-4"><p className="break-all">เลขออเดอร์: <strong>{order.orderId}</strong></p><p>ยอดจำลอง: ${order.total.toFixed(2)} · ไม่มีการชำระเงินจริง</p><button className="px-5 py-3 rounded-xl bg-[#2D5A27] text-white hover:bg-[#23471E]" onClick={finish}>กลับไปเลือกสินค้า</button></div> : <>
      ...
      <button disabled={busy || !cartItems.length} onClick={placeOrder} className="px-5 py-3 rounded-xl bg-[#2D5A27] text-white hover:bg-[#23471E] disabled:opacity-50">{busy ? 'กำลังบันทึก…' : 'สร้างออเดอร์ทดลอง — ไม่เสียเงินจริง'}</button>
    </>}
  </section>;
}
